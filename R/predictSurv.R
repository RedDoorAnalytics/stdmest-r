...
predictSurv1R <- function(t, X, betaX, b, ln_p = 0.0) {
  # Calculate survival
  S <- exp(-exp(as.numeric(X %*% t(betaX)) + b) * (t^exp(ln_p)))
  # Return survival
  return(S)
}
...
predictMeanSurv1R <- function(t, X, betaX, b, ln_p) {
  ...
  for (i in 1:nrow(betaX)) {
    output[, i] <- vapply(X = t, FUN = function(tt) mean(predictSurv1R(t = tt, X = X, betaX = betaX[i, , drop = FALSE], b = b, ln_p = ln_p[i, ])), FUN.VALUE = numeric(length = 1L))
  }
  output
}
