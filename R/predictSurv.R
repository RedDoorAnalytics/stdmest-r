...
predictMeanSurv1R <- function(t, X, betaX, b, ln_p) {
  if (nrow(betaX) != nrow(ln_p)) {
    stop("Dimensions of 'betaX' and 'ln_p' do not match.")
  }
...
predictMeanIntSurvR <- function(t, X, betaX, b, ln_p, varmarg, GHdata) {
  ...
  if (nrow(betaX) != nrow(varmarg)) {
    stop("Dimensions of 'betaX' and 'varmarg' do not match.")
  }
  output <- matrix(0, nrow = length(t), ncol = nrow(betaX))
  nn <- nrow(X)
  for (i in seq_len(nrow(betaX))) {
